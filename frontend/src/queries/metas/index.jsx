...
import { getMetasView } from "../../services/metas.services";
...
const MetasView = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await getMetasView();
  ...
  const handleSearchChange = (event) => {
    ...
    const filtered = data.filter((item) =>
      item.descripcionP.toLowerCase().includes(value.toLowerCase())
    );
  ...
  return (
    <Box m="20px">
      <Box display="flex" justifyContent="space-between" p={0}>
        <Header title="Relatorio de Metas" subtitle="Relatorio de Metas" />
