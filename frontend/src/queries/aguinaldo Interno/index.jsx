...
const AguinaldoInterno = () => {
  ...
  const columns = [
    ...
    // },
    {
      ...
      cellClassName: "name-column--cell",
      renderCell: (params) => {
        const salarioips = Number(params.row.salario_pagado) || 0;
    ...
    },
    {
      ...
      cellClassName: "name-column--cell",
      renderCell: (params) => {
        const anticipo = Number(params.row.total_a_pagar) || 0;
       const calc = anticipo / 2;
  ...
  const agruparYCalcularDatos = (datos) => {
    ...
    .map((item) => ({
      ...item,
      total_a_pagar: item.salario_pagado / 12, // Agregar el cálculo directamente
    }));
  };
  ...
  const handleExport = async () => {
    ...
    worksheet.columns = [
        ...
        { header: 'Anticipo', key: 'anticipo', width: 15 },
        { header: 'Saldo', key: 'saldo', width: 15 },
    ];
  
    // Preparar los datos a exportar
    const exportableData = filteredData.map((row) => {
        const { Funcionario, grupo, ips, cedula, total_a_pagar, salario_pagado } = row;
        const anticipo = (total_a_pagar / 2);
        const saldo = (total_a_pagar / 2);
        ...
        return {
            ...
            anticipo: anticipo.toLocaleString('es-ES'),
  ...
  return (
    <Box m="20px">
      <Box display="flex" justifyContent="space-between" p={0}>
        <Header
          title="Resumen Aguinaldo Interno"
          subtitle="Resumen Aguinaldo Interno"
        />
...
export default AguinaldoInterno;
