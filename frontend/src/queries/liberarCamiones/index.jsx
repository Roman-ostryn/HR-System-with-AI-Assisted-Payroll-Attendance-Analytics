...
import {putCamionesPedidos, getLiberarCamiones, updateSalidaStock, getOnePedido} from "../../services/pedidoVenta.services"
...
import ModalPaquetes from "../../modal/cargamento/modalPaquetes"; // Importar el modal de vista previa
import ModalEditarPedido from "../../modal/pedidoVenta/modalEditarPedido";
...
const LiberarCamion = () => {
  ...
  const fetchPrueba = async () => {
    try {
      const response = await getLiberarCamiones();
      setData(response);
...
const handleEditar = async () => {
  ...
  try {
    await new Promise((resolve) => setTimeout(resolve, 2000));
    const response = await getOnePedido(selectedRow.n_pedido, );
    // console.log("🚀 ~ handleEditar ~ response:", response)
  ...
  return (
    <Box m="20px">
      <Box display="flex" justifyContent="space-between" p={0}>
        <Header
          title="Aguardando Liberacion"
          subtitle="Liberar Carga"
        />
      ...
      <ModalEditarPedido
        ...
        dataNota={dataNota}
        // dataStock={dataStock}
        numeroNota={dataNota.n_pedido} // Cambié aquí para usar el registro seleccionado
      />
    </Box>
  );
};

export default LiberarCamion;
