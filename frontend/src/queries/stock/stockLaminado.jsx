...
import { getOne, getOneDatos, getStockLaminado} from "../../services/stock.services";
...
import ModalChapasColar from "../../modal/cargamento/modalChapasCargadasColar";
...
const Stock = () => {
  ...
  const [openColarModal, setOpenColarModal] = useState(false);
  ...
  const fetchPrueba = async () => {
    try {
      const response = await getStockLaminado();
      setData(response);
  ...
  const handleColar = async () => {
    const { serie } = selectedRow;
    console.log("🚀 ~ handleColar ~ selectedRow:", selectedRow)
    setRegistroColarModal(selectedRow);
    setOpenColarModal(true);
  ...
  const columns = [
    ...
    { type: "string", field: "serie", headerName: "Serie Fabrica", flex: 0.8 },
    { field: "cod", headerName: "Producto", flex: 2 ,cellClassName: "name-column--cell"},
  ...
  ^FO520,330^A0R,30,30^FD${item.peso}^FS
  ...
  ^FO520,1110^A0R,30,30^FD${item.peso}^FS
  ...
  ^FO520,1910^A0R,30,30^FD${item.peso}^FS
  ...
  const handlePrint5cm  = async () => {
    ...
    try {
      ...
      const descripcionLines = formatText(item.descripcion, 50);
        ...
        descripcionLines.forEach((line, index) => {
          zpl += `^FO85,${260 + index * 30}^A0N,28,26^FD${line}^FS\n`;
        });

        zpl += `
        ^FO85,${80 + descripcionLines.length * 30 + 30}^A0N,28,26^FD^FS
  ...
  const handleCloseColarModal = () => {
    setOpenColarModal(false);
    handleMenuClose();
  };
  ...
  return (
    ...
    marginLeft:"2%",
      ...
      <Box display="flex" justifyContent="end" p={0} sx={{ ml: 2, marginBottom: "-3vh" }}>
                <Box sx={{ 
                  marginRight: "1vh",
                  fontSize: "20px",
                  color:"rgb(206, 220, 0)",
      ...
      >
        ...
        <MenuItem onClick={handleColar}>Visualizar Chapas</MenuItem>
