...
const Palitero = () => {
  ...
  const [registro, setRegistro] = useState(null);
     const [errorMessage, setErrorMessage] = useState(''); // Estado para guardar posibles errores
       ...
       const [registrationSuccess, setRegistrationSuccess] = useState(false);
         ...
         const [openPalitero, setOpenPalitero] = useState(false);
  ...
  const fetchPrueba = async () => {
    try {
      ...
      const response = await getCaballetePal();
  ...
  const columns = [
    ...
    { field: "codigo", headerName: "Codigo", flex: 1, cellClassName: "name-column--cell"},
     { field: "descripcion", headerName: "Palitero", flex: 1.5 },
  ...
  //   }else{
    ...
    const handleClosePalitero = () => {
      setOpenPalitero(false);
      // fetchPrueba();
      // handleMenuClose();
    }
