...
import {putEstado} from "../../services/caballete.services"
import { getOneDatos } from '../../services/interfoliacion.services';
import { getChapas, getCaballeteView2 } from '../../services/ordenProduccion.services';
...
const SalidaAutoclave = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setFieldValue }) => {
      ...
      try {
        ...
        if (data.id_caballete === id_caballete) {
          throw new Error(`La chapa ${data.serie} ya se encuentra en este caballete`);
        }
        const response = await putEstado(data.id, edit);
  ...
  const fetchPrueba = async () => {
    try {
      const response = await getCaballeteView2();
      setData(response);
  ...
  return (
    ...
    {/* <Box m="20px"> */}
      <Box display="flex" justifyContent="space-between" p={0}>
        <Header title="Salida AutoClave " subtitle="Salida AutoClave" />
  ...
  export default SalidaAutoclave;
