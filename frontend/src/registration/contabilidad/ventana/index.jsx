...
doc.rect(134, 10, 71, 67);
    ...
    doc.text("Precio Unit.", 164, yPosition);
    ...
    doc.text(`${item.cantidad} M2`, 6.5, yPosition);

  // LINEA CANTIDAD
    doc.setLineWidth(0.3);
    doc.line(27, 82, 27, 282); // Línea vertical a la izquierda

    // LINEA PARTIDA
    doc.setLineWidth(0.3);
    doc.line(57, 82, 57, 282); // Línea vertical a la izquierda

    // LINEA DESCRIPCION
    doc.setLineWidth(0.3);
    doc.line(161, 82, 161, 292); // Línea vertical a la izquierda

        // LINEA PRECIO UNITARIO
        doc.setLineWidth(0.3);
        doc.line(187, 82, 187, 282); // Línea vertical a la izquierda
    // Concatenar descripción y artículo con "; "
let texto = `${item.descripcion}; ${item.articulo}`;
let maxLength = 51;

// Insertar salto de línea si el texto supera la longitud máxima
let textoFormateado =
  texto.length > maxLength
    ? texto.slice(0, maxLength) + "\n" + texto.slice(maxLength)
    : texto;

    doc.text(textoFormateado, 59, yPosition);
      doc.text(`${item.partida}`, 28.9, yPosition);
      // doc.text(`${item.descripcion}${"; "}${item.articulo}`, 59, yPosition);
      
      doc.text(`${item.precioU}`, 169, yPosition);
      doc.text(`${item.totales}`, 190, yPosition);
      // Ajustar la posición solo si hubo un salto de línea
yPosition += texto.length > maxLength ? 20 : 10;
    });
    ...
    doc.text(`${totalGeneral} USD`, 172,  288.5);
     ...
     doc.setFont("helvetica", "bold");
     doc.setFontSize(9);
     doc.text(` ${valuesToSend.totalC}${" M2"}`, 8, 220);
...
export default ExportacionVentana;