...
const getDescuento = async () => {
  try {
    const response = await api.get(`${API_URL}descuento/descuentoView`);
...
const getOneDescuento = async (id) => {
  try {
    const response = await api.get(`${API_URL}descuento/${id}`);
    return response.data;
  } catch (error) {
    throw new Error('Error al obtener datos del backend:', error);
  }
...
const PostDatosDescuento = async (datos) => {
  try {
    const response = await api.post(`descuento`, datos);
...
export { getDescuento, PostDatosDescuento, getOneDescuento, getListTruckView };