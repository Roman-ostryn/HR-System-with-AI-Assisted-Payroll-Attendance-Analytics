...
const getSalarios = async () => {
  try {
    const response = await api.get(`${API_URL}salarios`);
...
const getOneSalarios = async (id) => {
  try {
    const response = await axios.get(`${API_URL}salarios/${id}`);
    return response.data;
  } catch (error) {
    throw new Error('Error al obtener datos del backend:', error);
  }
...
const PostDatosSalarios = async (datos) => {
  try {
    const response = await api.post(`salarios`, datos);
    return response.data;
  } catch (error) {
...
export { getSalarios, PostDatosSalarios, getOneSalarios, getListTruckView };