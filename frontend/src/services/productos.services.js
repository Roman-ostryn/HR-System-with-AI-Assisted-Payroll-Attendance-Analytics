...
const getProductoProveedor = async (id) => {
  try {
    const response = await api.get(`productoProveedor/${id}`);
    return response.data;
  } catch (error) {
    throw new Error('Error al obtener datos del backend:', error);
  }
...
export { getDatosProducto, getOneDatos, getDatosCategoria, PostDatosProductos, getProductoProveedor };
