...
const getProveedor = async (codigo) => {
  try {
    const response = await api.get(`proveedor`, { params: { codigo } }); // Agrega el código como parámetro de consulta
    return response.data;
  } catch (error) {
    throw new Error('Error al obtener datos del backend:', error);
  }
...
const PostDatos = async (datos) => {
  try {
    const response = await api.post('proveedor', datos);
    return response.data;
  } catch (error) {
...
export { getDatosProveedor, PostDatos, getOneDatos, getProveedor };
