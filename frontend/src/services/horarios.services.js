...
const getOneHorarios = async (id) => {
  try {
    const response = await api.get(`${API_URL}horarios/${id}`);
    return response.data;
  } catch (error) {
    throw new Error('Error al obtener datos del backend:', error);
  }
...
const PostDatosHorarios = async (datos) => {
  try {
    const response = await api.post(`horarios`, datos);
...
export { getDatosHorarios, PostDatosHorarios, getOneHorarios, getListTruckView };