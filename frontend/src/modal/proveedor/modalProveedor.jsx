import React, { useState, useEffect } from 'react';
  ...
  import { getDatosProveedor } from '../../services/proveedor.services'; // Asegúrate de tener este servicio

  
  const ModalProveedor = ({ open, onClose, onSelect,}) => {
    ...
    const columns = [
      ...
      { field: "direccion", headerName: "Direccion", flex: 2 },
    ...
    useEffect(() => {
      ...
      const fetchProveedor = async () => {
        ...
        try {
          const response = await getDatosProveedor(); // Usa serviceType aquí
    ...
    const handleSearchChange = (event) => {
      ...
      const filtered = proveedor.filter((proveedors) =>
        proveedors.descripcion ? proveedors.descripcion.toLowerCase().includes(event.target.value.toLowerCase()) : false
      );
        ...
        >
          <Box m="0px">
            <Box display="flex" justifyContent="space-between" p={2} >
              <h1 id="owner-modal-title">Buscar Proveedor</h1>
              <Box
                display="flex"
