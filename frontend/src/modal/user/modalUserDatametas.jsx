  ...
  import { getDatosUser, getUserView } from '../../services/user.services'; // Asegúrate de tener este servicio

  const ModalUserDatametas = ({ open, onClose, onSelectUsuario, id_grupo }) => {
    ...
    const columns = [
      ...
      { field: "id_grupo", headerName: "Grupo", flex: 1 },
    ...
    useEffect(() => {
      const fetchUser = async () => {
        try {
          const response = await getUserView();
          setUsuarios(response);
          if (id_grupo) {
            const filtered = response.filter(user => user.id_grupo === id_grupo);
    ...
    const handleSearchChange = (event) => {
      ...
      const filtered = usuarios
        .filter(user => !id_grupo || user.id_grupo === id_grupo) // filter by group
        .filter(user =>
  ...
  export default ModalUserDatametas;
