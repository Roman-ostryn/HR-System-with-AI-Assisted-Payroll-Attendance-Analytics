...
import { getProblemaLavadora } from '../../services/reporteLavadora.services'; // Asegúrate de tener este servicio

const ModalProblemaLavadora = ({ open, onClose, onSelect }) => {
  const [problemasLavadora, setProblemasLavadora] = useState([]); // Aquí está definida 'problemasLavadora'
  ...
  useEffect(() => {
    const fetchProblemas = async () => {
      try {
        ...
        setProblemasLavadora(response); // Actualizar los datos recibidos
        setFilteredData(response); // Establecer datos filtrados iniciales
      } catch (error) {
        console.error('Error al obtener problemas de lavadora:', error);
      }
    };
    fetchProblemas();
  }, []);
  ...
  const handleSearchChange = (event) => {
    ...
    const filtered = problemasLavadora.filter((problema) =>
      ...
      >
        <Box m="0px">
          <Box display="flex" justifyContent="space-between" p={2}>
            <h1 id="owner-modal-title">Buscar Problema de Lavadora</h1>
            <Box
              display="flex"
