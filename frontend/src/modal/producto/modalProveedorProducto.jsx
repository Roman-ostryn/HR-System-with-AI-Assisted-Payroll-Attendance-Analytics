  ...
  import { getProductoProveedor } from '../../services/productos.services'; // Asegúrate de tener este servicio

  const ModalProductos = ({ open, onClose, onSelectProduct, serviceType }) => {
    ...
    useEffect(() => {
      setProveedores(serviceType);
    } , [serviceType]);

    const columns = [
      { field: "id", headerName: "ID", flex: 0.5 },
      { field: "cod", headerName: "Codigo", flex: 1.5 },
    ...
    useEffect(() => {
      ...
      const fetchProductos = async () => {
        try {
            const response = await getProductoProveedor(proveedores || 1);
