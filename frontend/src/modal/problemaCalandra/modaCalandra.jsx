...
import { getProblemaCalandra} from '../../services/reporteCalandra.services'; // Asegúrate de tener este servicio

const ModalProblemaCalandra= ({ open, onClose, onSelect }) => {
  const [problemasCalandra, setProblemasCalandra] = useState([]); // Aquí está definida 'problemasCalandra'
  ...
  useEffect(() => {
    const fetchProblemas = async () => {
      try {
        ...
        setProblemasCalandra(response); // Actualizar los datos recibidos
  ...
  const handleSearchChange = (event) => {
    ...
    const filtered = problemasCalandra.filter((problema) =>
      ...
      >
        <Box m="0px">
          <Box display="flex" justifyContent="space-between" p={2}>
            <h1 id="owner-modal-title">Buscar Problema de Calandra</h1>
            <Box
              display="flex"
...
export default ModalProblemaCalandra;
