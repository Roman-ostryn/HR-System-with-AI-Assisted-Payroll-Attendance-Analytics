...
import { getDatosInterfolacionColar, getIdColar } from '../../services/interfoliacion.services';
...
import { getDatosPalitero, getIdColarPalitero } from '../../services/palitero.services';
  ...
  const ModalPalitero = ({ open, onClose, onSelect, id }) => {
    ...
    const [searchValue, setSearchValue] = useState("");
     const [printerInfo, setPrinterInfo] = useState(''); // Estado para guardar la información de la impresora
       ...
       const [idPalitero, setIdPalitero] = useState(null);
    ...
    useEffect(() => {
      const fetchProductos = async () => {
        if (open) { // Solo hacer la solicitud si el modal está abierto
          try {
            // getDatosPalitero(idPalitero);
            const response = await getDatosPalitero(idPalitero); // Usa serviceType aquí
      ...
      // fetchProductos();
    }, [idPalitero]); // Dependencia en la prop 'open'
   ...
   useEffect(() => {
    ...
    const handleUpdate = async () => {
      ...
      await new Promise((resolve) => setTimeout(resolve, 1000));
       ...
       try {
        const response = await getIdColarPalitero(idPalitero); 
