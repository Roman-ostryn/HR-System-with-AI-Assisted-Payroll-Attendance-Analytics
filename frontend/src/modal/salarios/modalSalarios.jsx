  ...
  const ModalSalario = ({ open, onClose, onSelectSalario }) => {
    ...
    const handleSearchChange = (event) => {
      ...
      const filtered = salarios.filter((salario) =>
        salario.descripcion.toLowerCase().includes(event.target.value.toLowerCase())
      );
    ...
    const handleRowDoubleClick = (params) => {
      ...
      onSelectSalario({ id, descripcion, monto }); // Envía el salario seleccionado
    ...
    return (
      <Modal
        open={open}
        onClose={onClose}
        aria-labelledby="modal-salario-title"
        aria-describedby="modal-salario-description"
        ...
        >
          <Box m="0px">
            <Box display="flex" justifyContent="space-between" p={2} >
              <h1 id="owner-modal-title">Buscar Salarios</h1>
              <Box
                display="flex"
