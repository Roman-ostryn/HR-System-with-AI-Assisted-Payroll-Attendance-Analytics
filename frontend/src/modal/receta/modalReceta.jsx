  ...
  import { getDatosReceta } from '../../services/receta.services'; // Asegúrate de tener este servicio

  const ModalReceta = ({ open, onClose, onSelect }) => {
    ...
    const columns = [
      ...
      // { field: "id_categoria", headerName: "Categoria", flex: 1 },
    ...
    useEffect(() => {
      ...
      const fetchReceta = async () => {
        try {
          const response = await getDatosReceta();
          setReceta(response);
    ...
    const handleSearchChange = (event) => {
      ...
      const filtered = receta.filter((producto) =>
    ...
    const handleRowClick = (params) => {
      ...
      onSelect(selectedProducto); // Envía los datos del usuario seleccionado
        ...
        >
          <Box m="0px">
            <Box display="flex" justifyContent="space-between" p={2} >
              <h1 id="owner-modal-title">Buscar Producto</h1>
              <Box
                display="flex"
