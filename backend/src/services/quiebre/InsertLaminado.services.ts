...
export async function insertDataTestRe(tableName: string, data: Record<string, any>): Promise<any> {
  ...
  let {id, cantidad_entrada, cod, cod_interno, descripcion, id_notafiscal} = productData;


  if (descripcion.includes("VIDRO LAMINADO")) {
    id_notafiscal = 1;
  }
  ...
  let datosActualizados: Record<string, any> = {
    ...
    imagen: imagen,
    id_stock: productData.id
    };

  // const id_notafiscal = 1;
