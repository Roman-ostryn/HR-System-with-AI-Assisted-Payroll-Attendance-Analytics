...
export const updateData1 = async (id_marcacion: number, aprobacion_horas_extras: number) => {
  const entityManager = getManager();
  ...
  const result = await entityManager.query(
      `
    UPDATE marcacion
    SET aprobación_horas_extras = ?
    WHERE id = ?;
    `,
    [aprobacion_horas_extras, id_marcacion]
  );

  // Return the raw result for further handling
  return result;
};
