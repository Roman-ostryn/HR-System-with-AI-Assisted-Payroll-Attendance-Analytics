...
import { insertData} from "../services/descuento/Insert.services";
...
export const getDescuentosView = async (req: Request, res: Response) => {
  ...
  try {
    const userData = await getDataDes(tableName);
    res.json(userData);
  } catch (error) {
    console.error("Error getting product data:", error);
